...
static func calculate_storing_positions(count: int, spacing: float, direction: Vector3, origin: Node3D) -> Array:
    var out := []
    if count <= 0:
        return out
...
static func calculate_bank_positions(total_count: int, spacing: float, direction: Vector3, bank_origin: Node3D) -> Array:
    var out := []
    if total_count <= 0:
        return out
